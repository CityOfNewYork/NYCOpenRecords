...
def upgrade():
    ...
    op.create_table('custom_request_forms',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('agency_ein', sa.String(length=4), nullable=False),
    sa.Column('form_name', sa.String(), nullable=False),
    sa.Column('field_definition', postgresql.JSONB(astext_type=sa.Text()), nullable=False),
    sa.Column('repeatable', sa.Integer(), nullable=False),
    sa.ForeignKeyConstraint(['agency_ein'], ['agencies.ein'], ),
    sa.PrimaryKeyConstraint('id')
    )
    op.add_column('requests', sa.Column('custom_metadata', postgresql.JSONB(astext_type=sa.Text()), nullable=True))
