...
request_user_type_enum = postgresql.ENUM('requester', 'agency', name='request_user_type', create_type=False)
...
def upgrade():
    ...
    op.execute('create index if not exists users_lower_idx on users_new (lower(email::text));')
    op.execute('''INSERT INTO users_new (guid, is_super, first_name, middle_initial, last_name, email, email_validated, 
    ...
    op.execute('''ALTER TABLE agency_users DROP CONSTRAINT IF EXISTS agency_users_user_guid_fkey1;''')
    ...
    op.execute('create index if not exists users_lower_idx on users (lower(email::text));')
    op.execute('''INSERT INTO users (guid, is_super, first_name, middle_initial, last_name, email, email_validated, 
                        terms_of_use_accepted, title, organization, phone_number, fax_number, mailing_address, 
                        notification_email, session_id, signature, active, has_nyc_account, is_anonymous_requester, 
                        is_nyc_employee)
    ...
    # Add primary key to the Users
    op.create_table('user_requests_new',
    ...
    op.execute('''INSERT INTO user_requests_new (user_guid, request_id, permissions, 
    ...
    op.execute('''INSERT INTO user_requests (user_guid, request_id, permissions, 
                  request_user_type, point_of_contact)
    ...
    op.create_table('agency_users_new',
                    sa.Column('user_guid', sa.String(length=64), nullable=False),
    ...
    op.execute('''INSERT INTO agency_users_new (user_guid, agency_ein, is_agency_active, 
    ...
    op.execute('''INSERT INTO agency_users (user_guid, agency_ein, is_agency_active, 
                  is_agency_admin, is_primary_agency)
                  SELECT
                    user_guid,
                    agency_ein,
