...
from models import RecordPrivacy, Record
        ...
        if self.type == "offline":
            if current_user.role not in ['Agency Helpers'] and current_user.role is not None:
                if self.response.privacy == RecordPrivacy.RELEASED_AND_PUBLIC:
                    ...
                    return "How to Access Record: <br />%s <form method='post' action='/switchRecordPrivacy'><input type='hidden' id='form_id' value='switch_record_privacy'><input name=_csrf_token id='_csrf_token' type=hidden value='%s'><input type='hidden' name='request_id' id='request_id' value='%s'/><input type='hidden' name='record_id' id='record_id' value='%s'/><input class='radio inline privacy_radio' type='radio' name='privacy_setting' value='release_and_public' id='release_and_public'> Release and Public</input><input class='radio inline privacy_radio' type='radio' name='privacy_setting' value='release_and_private' id='release_and_private' type='submit' checked> Release and Private</input><input class='radio inline privacy_radio' type='radio' name='privacy_setting' value='private' type='submit' onclick='action=this.form.submit();'> Private</input></form>" % (
                        bleach.clean(self.response.access), session[
                            '_csrf_token'], self.response.request_id,
                        self.response.id)
                else:
            ...
            if current_user.role not in ['Agency Helpers'] and current_user.role is not None:
                if self.response.privacy == RecordPrivacy.RELEASED_AND_PUBLIC:
                    #Getting the size of the file here
                    url = app.config['UPLOAD_PUBLIC_LOCAL_FOLDER'] + "/" + self.response.request_id + "/"  + self.response.filename
                    readFile = open(url,'rb').read()
                    download_url = "/attachments/public/" + self.response.request_id + "/" + \
                        str(self.response.filename)
                    if len(readFile) < int(app.config['MAX_EMAIL_ATTACHMENT_SIZE']):
                        return "<a href='%s' rel='tooltip' data-toggle='tooltip' data-placement='top' data-original-title='%s' target='_blank'><i class='icon-file'></i></i>&nbsp;<b>%s </b><br />%s </a><form method='post' action='/switchRecordPrivacy'><input type='hidden' id='form_id' value='switch_record_privacy'><input name=_csrf_token id='_csrf_token' type=hidden value='%s'><input type='hidden' name='request_id' id='request_id' value='%s'/><input type='hidden' name='record_id' id='record_id' value='%s'/><input class='radio inline privacy_radio' type='radio' name='privacy_setting' value='release_and_public' id='release_and_public' checked> Release and Public</input><input class='radio inline privacy_radio' type='radio' name='privacy_setting' value='release_and_private' id='release_and_private' type='submit'> Release and Private</input><input class='radio inline privacy_radio' type='radio' name='privacy_setting' value='private' type='submit' onclick='action=this.form.submit();'> Private</input><input class='addAsEmail' type='checkbox' id='addEmailAttachment' name='addAsEmailAttachment_1' value='on' style='display:inline-block;width:1em;margin-bottom:0.4em;''>Add to Email </form>" % (download_url, self.response.description, self.response.description, self.response.filename, session['_csrf_token'], self.response.request_id, self.response.id)
                    else:
                        return "<a href='%s' rel='tooltip' data-toggle='tooltip' data-placement='top' data-original-title='%s' target='_blank'><i class='icon-file'></i></i>&nbsp;<b>%s </b><br />%s </a><form method='post' action='/switchRecordPrivacy'><input type='hidden' id='form_id' value='switch_record_privacy'><input name=_csrf_token id='_csrf_token' type=hidden value='%s'><input type='hidden' name='request_id' id='request_id' value='%s'/><input type='hidden' name='record_id' id='record_id' value='%s'/><input class='radio inline privacy_radio' type='radio' name='privacy_setting' value='release_and_public' id='release_and_public' checked> Release and Public</input><input class='radio inline privacy_radio' type='radio' name='privacy_setting' value='release_and_private' id='release_and_private' type='submit'> Release and Private</input><input class='radio inline privacy_radio' type='radio' name='privacy_setting' value='private' type='submit' onclick='action=this.form.submit();'> Private</input></form>" % (download_url, self.response.description, self.response.description, self.response.filename, session['_csrf_token'], self.response.request_id, self.response.id)
                elif self.response.privacy == RecordPrivacy.RELEASED_AND_PRIVATE:
                    url = app.config['UPLOAD_PRIVATE_LOCAL_FOLDER'] + "/" + self.response.request_id + "/"  + self.response.filename
                    readFile = open(url,'rb').read()
                    download_url = "/attachments/private/" + self.response.request_id + "/" + \
                        str(self.response.filename)
                    if len(readFile) < int(app.config['MAX_EMAIL_ATTACHMENT_SIZE']):
                        return "<a href='%s' rel='tooltip' data-toggle='tooltip' data-placement='top' data-original-title='%s' target='_blank'><i class='icon-file'></i></i>&nbsp;<b>%s </b><br />%s </a><form method='post' action='/switchRecordPrivacy'><input type='hidden' id='form_id' value='switch_record_privacy'><input name=_csrf_token id='_csrf_token' type=hidden value='%s'><input type='hidden' name='request_id' id='request_id' value='%s'/><input type='hidden' name='record_id' id='record_id' value='%s'/><input class='radio inline privacy_radio' type='radio' name='privacy_setting' value='release_and_public' id='release_and_public'> Release and Public</input><input class='radio inline privacy_radio' type='radio' name='privacy_setting' value='release_and_private' id='release_and_private' type='submit' checked> Release and Private</input><input class='radio inline privacy_radio' type='radio' name='privacy_setting' value='private' type='submit' onclick='action=this.form.submit();'> Private</input><input class='addAsEmail' type='checkbox' id='addEmailAttachment' name='addAsEmailAttachment_1' value='on' style='display:inline-block;width:1em;margin-bottom:0.4em;''>Add to Email </form>" % (download_url, self.response.description, self.response.description, self.response.filename, session['_csrf_token'], self.response.request_id, self.response.id)
                    else:
                    ...
                    download_url = "/attachments/private/" + self.response.request_id + "/" + \
                        str(self.response.filename)
                    if len(readFile) < int(app.config['MAX_EMAIL_ATTACHMENT_SIZE']):
                        return "<a href='%s' rel='tooltip' data-toggle='tooltip' data-placement='top' data-original-title='%s' target='_blank'><i class='icon-file'></i></i>&nbsp;<b>%s </b><br />%s </a><form method='post' action='/switchRecordPrivacy'><input type='hidden' id='form_id' value='switch_record_privacy'><input name=_csrf_token id='_csrf_token' type=hidden value='%s'><input type='hidden' name='request_id' id='request_id' value='%s'/><input type='hidden' name='record_id' id='record_id' value='%s'/><input class='radio inline privacy_radio' type='radio' name='privacy_setting' value='release_and_public' id='release_and_public'> Release and Public</input><input class='radio inline privacy_radio' type='radio' name='privacy_setting' value='release_and_private' id='release_and_private' type='submit'> Release and Private</input><input class='radio inline privacy_radio' type='radio' name='privacy_setting' value='private' type='submit' onclick='action=this.form.submit();' checked> Private</input><input class='addAsEmail' type='checkbox' id='addEmailAttachment' name='addAsEmailAttachment_1' value='on' style='display:inline-block;width:1em;margin-bottom:0.4em;''>Add to Email </form>" % (download_url, self.response.description, self.response.description, self.response.filename, session['_csrf_token'], self.response.request_id, self.response.id)
                    else:
            ...
            if self.response.description and self.response.url and not (self.response.filename):
                ...
                if current_user.role not in ['Agency Helpers'] and current_user.role is not None:
                    if self.response.privacy == RecordPrivacy.RELEASED_AND_PUBLIC:
                        ...
                        return "<a href='%s' rel='tooltip' data-toggle='tooltip' data-placement='top' data-original-title='%s'><i class='icon-share'></i>&nbsp;<b>%s </b></a><form method='post' action='/switchRecordPrivacy'><input type='hidden' id='form_id' value='switch_record_privacy'><input name=_csrf_token id='_csrf_token' type=hidden value='%s'><input type='hidden' name='request_id' id='request_id' value='%s'/><input type='hidden' name='record_id' id='record_id' value='%s'/><input class='radio inline privacy_radio' type='radio' name='privacy_setting' value='release_and_public' id='release_and_public'> Release and Public</input><input class='radio inline privacy_radio' type='radio' name='privacy_setting' value='release_and_private' type='submit' onclick='action=this.form.submit();' checked> Release and Private</input><input class='radio inline privacy_radio' type='radio' name='privacy_setting' value='private' type='submit' onclick='action=this.form.submit();'> Private</input></form>" % (
                            bleach.clean(self.response.url), bleach.clean(
                                self.response.url),
                            bleach.clean(self.response.description), session[
                                '_csrf_token'], self.response.request_id,
            ...
            if self.response.days_after is not None and self.response.due_date is not None:
                if self.response.days_after == -1:
                    text = "A custom due date was set. The request is now due on " + datetime.datetime.strftime(self.response.due_date, '%m/%d/%Y')
                else:
